...
def run_couple_training(cleaned_file: Path) -> Dict[str, Any]:
    ...
    with safe_start_run(run_name) as run:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Adapt results structure - CORRECTION ICI
            linear_results = []
            nonlinear_results = []
            ...
            # ============================================
            # PROCESS LINEAR MODELS - CORRECTION ICI
            # ============================================
            ...
            for idx, lin in enumerate(linear_results):
                try:
                    ...
                    if pipeline is None:
                        print(f"   ⚠️ No pipeline found for {group}")
                        continue
                    ...
                    if formula:
                        model_metadata["formula"] = formula

                    # Sauvegarder le pipeline temporairement
                    pipeline_file = tmp_path / f"{group}_linear_pipeline.joblib"
            ...
            # ============================================
            # PROCESS NONLINEAR MODELS - CORRECTION ICI
            # ============================================
            ...
            for idx, nl in enumerate(nonlinear_results):
                try:
                    ...
                    # Sauvegarder le pipeline temporairement
                    pipeline_file = tmp_path / f"{group}_nonlinear_pipeline.joblib"
                    dump(pipeline, pipeline_file)

                    # Prepare feature importance for tags
                    feature_importance_json = None
                    if feature_importance:
                        limited_fi = dict(list(feature_importance.items())[:50])
                        feature_importance_json = json.dumps(limited_fi)
                ...
                "linear_pairs": [
                    {
                        ...
                        "cv_r2": float(lin.get("cv_r2", 0)),
                        "n_samples": int(lin.get("n_samples", 0)),
                        "model_name": f"PricingModel_{lin.get('binding_type', '')}__{lin.get('siren', '')}_Linear" if lin.get('binding_type') and lin.get('siren') else f"PricingModel_{lin.get('group', f'pair_{i}')}_Linear",
                ...
                "nonlinear_pairs": [
                    {
                        ...
                        "r2": float(nl.get("r2_test", nl.get("r2", 0))),
                        "n_samples": int(nl.get("n_samples", 0)),
                        "model_name": f"PricingModel_{nl.get('binding_type', '')}__{nl.get('siren', '')}_NonLinear" if nl.get('binding_type') and nl.get('siren') else f"PricingModel_{nl.get('group', f'pair_{i}')}_NonLinear",
                        "version": next((m["version"] for m in created_models if
...
def pricing_mlops_pipeline(
    ...
    print("\n📋 Configuration:")
    print(f"   - Consolidation: {'YES' if run_consolidation else 'NO'}")
    print(f"   - Client Features: {'YES' if run_client_features else 'NO'}")
    print(f"   - Global Model: {'YES' if run_global else 'NO'}")
    print(f"   - BindingType: {'YES' if run_family else 'NO'}")
    ...
    if run_client_features:
        ...
        try:
            ...
            # Display MLflow link
            if "run_id" in mlflow_result:
                print(
                    f"\n🔗 MLflow link: {mlflow.get_tracking_uri()}/#/experiments/{mlflow_result['experiment_id']}/runs/{mlflow_result['run_id']}")

            # Use enriched data for subsequent steps if available
            if enriched_path != cleaned_path:
                training_data_path = enriched_path
                print(f"✅ Using enriched data for training")

                # Update sample_X with new features
                if not enriched_df.empty:
                    ...
                    if available_features:
                        ...
                        print(f"✅ Sample_X updated with {len(available_features)} features")
        except Exception as e:
            print(f"❌ Failed to create client features: {e}")
            traceback.print_exc()
            results["client_features"] = {"error": str(e)}
    else:
        print("\n⏩ Step 3 skipped")
    ...
    if run_global:
        print("\n" + "=" * 60)
        print("🌍 STEP 4/6: GLOBAL MODEL TRAINING")
        print("=" * 60)
        try:
            ...
            traceback.print_exc()
            results["global"] = {"error": str(e), "model_name": MLFLOW_MODEL_NAME_GLOBAL}
    else:
        print("\n⏩ Step 4 skipped")

    # STEP 5: BINDINGTYPE MODELS
    if run_family:
        print("\n" + "=" * 60)
        print("🏷️ STEP 5/6: TRAINING MODELS BY BINDINGTYPE")
        print("=" * 60)
        try:
    ...
    if run_couple:
        print("\n" + "=" * 60)
        print("🔗 STEP 6/6: TRAINING MODELS BY PAIR")
        print("=" * 60)
        try:
            ...
            traceback.print_exc()
            results["couple"] = {"error": str(e)}
    else:
        print("\n⏩ Step 6 skipped")

    # STEP 7: PRODUCTION ALIAS CONFIGURATION
    if promote and not once:
        print("\n" + "=" * 60)
        print("🏆 STEP 7/6: CONFIGURING PRODUCTION ALIAS")
        print("=" * 60)

        global_to_promote = None
        family_to_promote = None
        couple_to_promote = None
        client_features_to_promote = None

        if run_global and "global" in results and "error" not in results["global"]:
            global_to_promote = results["global"]
            print(f"\n✅ Global model ready for configuration")

        if run_family and "family" in results and "error" not in results["family"]:
            family_to_promote = results["family"]
            print(f"✅ {len(results['family'].get('created_models', []))} family models ready")

        if run_couple and "couple" in results and "error" not in results["couple"]:
            couple_to_promote = results["couple"]
            print(f"✅ {len(results['couple'].get('created_models', []))} pair models ready")

        if run_client_features and client_features_result is not None:
            client_features_to_promote = client_features_result
            print(f"✅ Client features ready (v{client_features_result.get('model_version', 'N/A')})")

        if global_to_promote or family_to_promote or couple_to_promote or client_features_to_promote:
            promote_results = promote_models(
                global_to_promote,
                family_to_promote,
                couple_to_promote,
                client_features_to_promote
            )
            results["alias_config"] = promote_results
    ...
    print(f"\n⏱️  Total duration: {duration:.1f} minutes")

    # Client features summary
    if run_client_features and "client_features" in results and "error" not in results["client_features"]:
        print(f"\n👥 CLIENT FEATURES:")
        print(f"   Clients: {results['client_features']['n_clients']}")
        if 'stats' in results['client_features'] and 'elasticity_mean' in results['client_features']['stats']:
            print(f"   Average elasticity: {results['client_features']['stats']['elasticity_mean']:.3f}")
        if 'stats' in results['client_features'] and 'clients_with_elasticity' in results['client_features']['stats']:
            print(f"   Clients with elasticity: {results['client_features']['stats']['clients_with_elasticity']}")
        if 'mlflow' in results['client_features']:
            mlflow_info = results['client_features']['mlflow']
            print(f"   MLflow Run: {mlflow_info.get('run_id', 'N/A')}")
            print(f"   Model: {mlflow_info.get('model_name', 'N/A')} v{mlflow_info.get('model_version', 'N/A')}")
            if promote and "alias_config" in results and results["alias_config"]["client_features"]["success"]:
    ...
    if run_global and "global" in results and "error" not in results["global"]:
        print(f"\n🌍 GLOBAL MODEL:")
        print(f"   Best model: {results['global']['best_model_name']}")
        print(f"   Version: {results['global'].get('model_version', 'N/A')}")
        if promote:
            prod_version = get_model_version_by_alias(MLFLOW_MODEL_NAME_GLOBAL, ALIAS_PRODUCTION)
    ...
    print("\n" + "=" * 80)
    print("✅ PIPELINE COMPLETED".center(80))
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-client-features", action="store_true",
                        help="Do not create client features")
    parser.add_argument("--clean", action="store_true",
                        help="Clean artifacts before execution")
    parser.add_argument("--once", action="store_true",
                        help="One-shot mode (no promotion)")
    parser.add_argument("--no-promote", action="store_true",
                        help="Do not configure aliases automatically")
    ...
    if args.mode == "global":
        results = pricing_mlops_pipeline(
