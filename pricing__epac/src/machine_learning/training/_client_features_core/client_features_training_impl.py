...
def normalize_reception_dates(series: pd.Series) -> pd.Series:
    """Coerce mixed reception date values into a comparable datetime series."""
    cleaned = series.copy()
    ...
    remaining_mask = parsed.isna() & cleaned.notna()
    if remaining_mask.any():
        parsed.loc[remaining_mask] = pd.to_datetime(cleaned.loc[remaining_mask], errors="coerce", dayfirst=True)
    return parsed
...
def aggregate_client_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'reception_date' in df.columns:
        df['reception_date'] = normalize_reception_dates(df['reception_date'])
        invalid_dates = int(df['reception_date'].isna().sum())
        if invalid_dates:
            logger.warning("Coerced %s invalid reception_date value(s) to NaT before aggregation", invalid_dates)
