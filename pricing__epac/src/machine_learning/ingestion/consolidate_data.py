...
# Directory paths
PROJECT_ROOT = Path(os.getenv('PROJECT_ROOT', settings.PROJECT_ROOT))
SQL_DUMPS_DIR = Path(os.getenv('SQL_DUMPS_DIR', settings.SQL_WATCH_DIR))
STATIC_DATA = Path(os.getenv('STATIC_DATA', settings.DATA_ROOT / "raw" / "static" / "epac_historiquee.csv"))
CONSOLIDATED_FILE = Path(os.getenv('CONSOLIDATED_FILE', settings.DATA_ROOT / "consolidated" / "dataset_complet.xlsx"))
TRACKING_FILE = Path(os.getenv('TRACKING_FILE', settings.WATCHER_TRACKING_FILE))
